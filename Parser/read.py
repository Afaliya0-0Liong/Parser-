...
def get_code():
    line = []
    code = []
    with open("./code/out_from_task1.txt") as f:
...
def read(action, goto, lr1):
    imfo, sta, imfo_line = analyse(action, goto, lr1)
    with open("./result.txt", "w") as f:
        if sta:
            ...
            if type(imfo) == type(False):
                print('NO')
                a = '错误行： ' + imfo_line + '，\t是否未输入全？'
                f.write(a)
                print(a)
            else:
                print('NO')
                a = '错误行： ' + imfo_line + '\t' + list(vn.keys())[list(vn.values()).index(imfo[0])] + "后面不可以接: " + list(vn.keys())[list(vn.values()).index(imfo[1])]
                f.write(a)
                print(a)
